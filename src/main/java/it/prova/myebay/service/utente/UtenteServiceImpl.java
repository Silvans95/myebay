...
//	########################################################################
	...
	@Override
	public void inserisciNuovoConRuoli(Utente utenteInstance, String[] ruoliInstance) throws Exception {
		// questo è come una connection
		EntityManager entityManager = LocalEntityManagerFactoryListener.getEntityManager();
		...
		try {
			...
			for (int i = 0; i < ruoliInstance.length; i++) {
				Long idRuoli = Long.parseLong(ruoliInstance[i]);

				ruoli.add(ruoloDAO.findOne(idRuoli).orElse(null));
			}
